      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-10 gap-4">
          <div className="flex gap-2 items-center">
            {getPageNumbers()[0] > 1 && (
              <>
                <button onClick={() => setCurrentPage(1)} className={`w-12 h-12 rounded-md text-sm cursor-pointer border-2 border-yellow-400 ${currentPage === 1 ? 'bg-dark text-yellow-400 font-bold' : 'bg-white text-black'}`}>
                  1
                </button>
            ...
            {getPageNumbers().map((page) => (
              <button
                ...
                className={`w-12 h-12 rounded-md text-sm cursor-pointer border-2 border-yellow-400 ${currentPage === page ? 'bg-red-500 text-yellow-400 font-bold' : 'bg-white text-black'}`}
              >
                {page}
              </button>
            ))}
            ...
            {getPageNumbers().slice(-1)[0] < totalPages && (
              <>
                ...
                <button
                  ...
                  className={`w-12 h-12 rounded-md text-sm cursor-pointer border-2 border-yellow-400 ${currentPage === totalPages ? 'bg-dark text-yellow-400 font-bold' : 'bg-white text-black'}`}
                >
                  {totalPages}
                </button>
              </>
