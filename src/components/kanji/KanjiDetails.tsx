    ...
    <div className="z-50 flex flex-col space-y-10">
      <div className="flex space-x-10 z-50">
        <div className="p-8 bg-red-500 text-yellow-400 font-bold rounded-md ">
          <h1 className="text-7xl">{kanji}</h1>
        </div>
        <div>
          <div id="kunyomi">
      ...
      <div className="z-50">
        ...
        <ul className="grid gap-6 grid-cols-[repeat(auto-fit,minmax(120px,1fr))] max-md:grid-cols-[repeat(auto-fit,minmax(100px,1fr))]">
          {kanjiDetails?.kanji.strokes.images.map((image, idx) => (
            <li key={idx} className="relative">
              ...
              <img src={image} alt={`stroke-${idx}`} className="w-auto h-auto bg-yellow-400  rounded-md p-4" />
            </li>
          ))}
        </ul>
      </div>
